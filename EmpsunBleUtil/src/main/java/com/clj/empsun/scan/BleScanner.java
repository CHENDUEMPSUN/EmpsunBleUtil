...
import com.clj.empsun.callback.BleScanPresenterImp;
...
public class BleScanner {
            ...
            if (mBleScanPresenter.ismNeedConnect()) {
                ...
                if (bleDeviceList == null || bleDeviceList.size() < 1) {
                    ...
                    new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            EmpBleManager.getInstance().connect(list.get(0), callback);
                        }
                    }, 100);
                }
            } else {
    ...
    private synchronized void startLeScan(UUID[] serviceUuids, String[] names, String mac, boolean fuzzy,
        ...
        boolean success = EmpBleManager.getInstance().getBluetoothAdapter()
    ...
    public synchronized void stopLeScan() {
        EmpBleManager.getInstance().getBluetoothAdapter().stopLeScan(mBleScanPresenter);
