...
import com.clj.empsun.bluetooth.BleBluetooth;
import com.clj.empsun.bluetooth.MultipleBluetoothController;
import com.clj.empsun.bluetooth.SplitWriter;
import com.clj.empsun.callback.BleConnectNotifyImp;
import com.clj.empsun.callback.BleGattCallback;
import com.clj.empsun.callback.BleIndicateCallback;
import com.clj.empsun.callback.BleMtuChangedCallback;
import com.clj.empsun.callback.BleNotifyCallback;
import com.clj.empsun.callback.BleReadCallback;
import com.clj.empsun.callback.BleRssiCallback;
import com.clj.empsun.callback.BleScanAndConnectCallback;
...
import com.clj.empsun.data.BleScanState;
import com.clj.empsun.data.Constant;
import com.clj.empsun.exception.BleException;
import com.clj.empsun.exception.OtherException;
import com.clj.empsun.scan.BleScanRuleConfig;
import com.clj.empsun.scan.BleScanner;
import com.clj.empsun.utils.BleLog;
import com.clj.empsun.utils.SPUtils;

import java.util.List;
import java.util.UUID;

@TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR2)
public class EmpBleManager {
    ...
    public static EmpBleManager getInstance() {
        return BleManagerHolder.sBleManager;
    }

    private static class BleManagerHolder {
        private static final EmpBleManager sBleManager = new EmpBleManager();
    }
     ...
     *
     * @param count
     * @return EmpBleManager
     */
    public EmpBleManager setMaxConnectCount(int count) {
     ...
     * @return EmpBleManager
     */
    public EmpBleManager setOperateTimeout(int count) {
     ...
     * @return EmpBleManager
     */
    public EmpBleManager setReConnectCount(int count) {
     ...
     * @return EmpBleManager
     */
    public EmpBleManager setReConnectCount(int count, long interval) {
        if (count > 10)
            count = 10;
        if (interval < 0)
            interval = 0;
     ...
     * @return EmpBleManager
     */
    public EmpBleManager setSplitWriteNum(int num) {
     ...
     * @return EmpBleManager
     */
    public EmpBleManager setConnectOverTime(long time) {
        if (time <= 0) {
            time = 100;
        }
     ...
     * @return EmpBleManager
     */
    public EmpBleManager enableLog(boolean enable) {
    ...
    public String getSpMac() {
        return SPUtils.getString(Constant.DEVICE);
    }

    public void setSpMac(String value) {
        SPUtils.save(Constant.DEVICE, value);
    }
    ...
    public void startConnect(String mac, final BleConnectNotifyImp imp) {
        connect(mac, new BleGattCallback() {
            @Override
            ...
            public void onConnectFail(BleDevice bleDevice, BleException exception) {
                imp.onConnectFail(bleDevice, exception);
            }

            @Override
            public void onConnectSuccess(BleDevice bleDevice, BluetoothGatt gatt, int status) {
                SPUtils.save(Constant.DEVICE, bleDevice.getMac());
                imp.onConnectSuccess(bleDevice, gatt, status);
            }

            @Override
            public void onDisConnected(boolean isActiveDisConnected, BleDevice device, BluetoothGatt gatt, int status) {
                if (!isActiveDisConnected) {
                    Log.d("接收数据","重连了"+device.getMac());
                    startConnect(device.getMac(), imp);
                }
                imp.onConnectDis(isActiveDisConnected, device, gatt, status);
            }
        });
    }
    ...
    public void notify(BleDevice bleDevice,
        ...
        if (bleBluetooth == null) {
            ...
            bleBluetooth.newBleConnector()
                    ...
                    .enableCharacteristicNotify(callback, uuid_notify, bleDevice);
        }
    }
...
//    private void initMoreNotify(final BleDevice b, final List<String> list) {
//        EmpBleManager.getInstance().notify(b, list.get(0), list.get(1), new BleNotifyCallback() {
//            @Override
//            public void onNotifySuccess() {
//                BleLog.d("onNotifySuccess master:" + list.get(0) + "  slave:" + list.get(1));
...
//                BleLog.d("onNotifyFailure master:" + list.get(0) + "  slave:" + list.get(1));
//                if (mBleNotifyImp != null)
...
//                BleLog.d("onCharacteristicChanged master:" + list.get(0) + "  slave:" + list.get(1));
//                BleLog.d(list.get(0) + "--" + list.get(1) + " data:" + HexUtil.formatHexString(data));
//                if (mBleNotifyImp != null)
