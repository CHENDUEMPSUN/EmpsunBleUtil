package com.clj.empsun.bluetooth;
...
import com.clj.empsun.data.BleConnectStateParameter;
import com.clj.empsun.data.BleDevice;
import com.clj.empsun.data.BleMsg;
import com.clj.empsun.exception.ConnectException;
import com.clj.empsun.exception.OtherException;
import com.clj.empsun.exception.TimeoutException;
...
public class BleBluetooth {
    ...
    public synchronized BluetoothGatt connect(BleDevice bleDevice,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            bluetoothGatt = bleDevice.getDevice().connectGatt(EmpBleManager.getInstance().getContext(),
        ...
        if (bluetoothGatt != null) {
            ...
            mainHandler.sendMessageDelayed(message, EmpBleManager.getInstance().getConnectOverTime());
            ...
            EmpBleManager.getInstance().getMultipleBluetoothController().removeConnectingBle(BleBluetooth.this);
            if (bleGattCallback != null)
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case BleMsg.MSG_CONNECT_FAIL: {
                    ...
                    if (connectRetryCount < EmpBleManager.getInstance().getReConnectCount()) {
                        BleLog.e("Connect fail, try reconnect " + EmpBleManager.getInstance().getReConnectInterval() + " Millisecond later");

                        connectRetryCount++;

                        Message message = mainHandler.obtainMessage();
                        message.what = BleMsg.MSG_RECONNECT;
                        mainHandler.sendMessageDelayed(message, EmpBleManager.getInstance().getReConnectInterval());
                    } else {
                        connectRetryCount = 0;
                ...
                case BleMsg.MSG_DISCONNECTED: {
                    ...
                    EmpBleManager.getInstance().getMultipleBluetoothController().removeBleBluetooth(BleBluetooth.this);
                ...
                case BleMsg.MSG_DISCOVER_SUCCESS: {
                    ...
                    EmpBleManager.getInstance().getMultipleBluetoothController().removeConnectingBle(BleBluetooth.this);
                    EmpBleManager.getInstance().getMultipleBluetoothController().addBleBluetooth(BleBluetooth.this);
